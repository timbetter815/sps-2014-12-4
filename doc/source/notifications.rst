...
* ``image.create``
  ...
  Emitted when an image record is created in sps.  Image record creation is
  independent of image data upload.

* ``image.prepare``

  Emitted when sps begins uploading image data to its store.

* ``image.upload``

  Emitted when sps has completed the upload of image data to its store.

* ``image.activate``

  Emitted when an image goes to `active` status.  This occurs when sps
...
* ``image.update``
  ...
  Emitted when an image record is updated in sps.

* ``image.delete``

  Emitted when an image deleted from sps.
...
* publisher_id
  ...
  The hostname of the sps instance that generated the message.
