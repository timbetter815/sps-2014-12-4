...
sps has a number of options that you can use to configure the sps API
server, the sps Registry server, and the various storage backends that
sps can use to store demos.

Most configuration is done via configuration files, with the sps API
server and sps Registry server using separate configuration files.

When starting up a sps server, you can specify the configuration file to
use (see :doc:`the documentation on controller sps servers <controllingservers>`).
If you do **not** specify a configuration file, sps will look in the following
...
The sps API server configuration file should be named ``sps-api.conf``.
Similarly, the sps Registry server configuration file should be named
``sps-registry.conf``. If you installed sps via your operating system's
package management system, it is likely that you will have sample
configuration files installed in ``/etc/sps``.

In addition to this documentation page, you can check the
``etc/sps-api.conf`` and ``etc/sps-registry.conf`` sample configuration
files distributed with sps for example configuration files for each server
...
For example, ``sps-api-paste.ini`` corresponds to ``sps-api.conf``.
...
Common Configuration Options in sps
--------------------------------------

sps has a few command-line options that are common to all sps programs:
...
An example usage would be:
  ...
  $ sps-api --config-dir=/etc/sps/sps-api.d

  $ ls /etc/sps/sps-api.d
   00-core.conf
   01-s3.conf
   02-swift.conf
   03-ssl.conf
...
You can put the following options in the ``sps-api.conf`` and
``sps-registry.conf`` files, under the ``[DEFAULT]`` section. They enable
...
Number of sps API worker processes to start. Each worker
...
manually run `sps-manage db sync`.
...
There are a number of configuration options in sps that control how
...
``sps_CLIENT_KEY_FILE`` environ variable to a filepath of the key file
...
registry server, if any. Alternately, you may set the
``sps_CLIENT_CERT_FILE`` environ variable to a filepath of the cert file
...
registry server, if any. Alternately, you may set the
``sps_CLIENT_CA_FILE`` environ variable to a filepath of the CA cert file
...
specifying --insecure on the command line using spsclient for the API
...
Configuring Logging in sps
-----------------------------

There are a number of configuration options in sps that control how sps
...
for the API server, in a configuration file called ``etc/sps-api.conf``::

  [DEFAULT]
  log_file = /var/log/sps/api.log

* ``log_file``

The filepath of the file to use for logging messages from sps's servers. If
missing, the default is to output messages to ``stdout``, so if you are running
sps servers in a daemon mode (using ``sps-control``) you should make
...
Configuring sps Storage Backends
-----------------------------------

There are a number of configuration options in sps that control how sps
stores disk demos. These configuration options are specified in the
``sps-api.conf`` config file in the section ``[DEFAULT]``.
...
Sets the storage backend to use by default when storing demos in sps.
Available options for this option are (``file``, ``swift``, ``s3``, ``rbd``, ``sheepdog``, 
``cinder`` or ``vsphere``).

Configuring sps Image Size Limit
-----------------------------------
...
Maximum image size, in bytes, which can be uploaded through the sps API server.

**IMPORTANT NOTE**: this value should only be increased after careful consideration
and must be set to a value under 8 EB (9223372036854775808).

Configuring sps User Storage Quota
-------------------------------------
...
Optional. Default: ``/var/lib/sps/demos/``

Can only be specified in configuration files.

`This option is specific to the filesystem storage backend.`

Sets the path where the filesystem storage backend write disk demos. Note that
the filesystem storage backend will attempt to create this directory if it does
not exist. Ensure that the user that ``sps-api`` runs under has write
...
Optional. Default: ``/var/lib/sps/demos/:1``

Example::

  filesystem_store_datadirs = /var/sps/store
  filesystem_store_datadirs = /var/sps/store1:100
  filesystem_store_datadirs = /var/sps/store2:200
...
multiple store directories to save sps image in filesystem storage backend.
...
Optional. Default: ``sps``

Can only be specified in configuration files.

`This option is specific to the Swift storage backend.`

Sets the name of the container to use for sps demos in Swift.
...
If true, sps will attempt to create the container ``swift_store_container``
...
What size, in MB, should sps start chunking image files
...
sps write chunks to Swift?  The default is 200MB.
...
If set to True enables multi-tenant storage mode which causes sps demos
to be stored in tenant specific Swift accounts. When set to False sps
stores all demos in a single Swift account.
...
write ACLs to the containers created by sps in multi-tenant
...
example: swift_store_admin_tenants = service:sps,*:admin
...
requests. Setting to 'False' may improve performance for demos which
...
Sets the name of the bucket to use for sps demos in S3.
...
If true, sps will attempt to create the bucket ``s3_store_bucket``
if it does not exist.
...
When sending demos to S3, what directory should be
...
Sets the RADOS pool in which demos are stored.
...
demos will be chunked into objects of this size (in megabytes).
...
configuration file, e.g. with a user ``sps``::

  [client.sps]
  keyring=/etc/sps/rbd.keyring

To set up a user named ``sps`` with minimal permissions, using a pool called
``demos``, run::

  rados mkpool demos
  ceph-authtool --create-keyring /etc/sps/rbd.keyring
  ceph-authtool --gen-key --name client.sps --cap mon 'allow r' --cap osd 'allow rwx pool=demos' /etc/sps/rbd.keyring
  ceph auth add client.sps -i /etc/sps/rbd.keyring
...
Optional. Default: ``/openstack_sps``

Can only be specified in configuration files.

The path to access the folder where the demos will be stored in the datastore.
...
sps API servers can be configured to have a local image cache. Caching of
...
The cache middleware should be in your ``sps-api-paste.ini`` in a section
titled ``[filter:cache]``. It should look like this::

  [filter:cache]
  paste.filter_factory = sps.api.middleware.cache:CacheFilter.factory

A ready-made application pipeline including this filter is defined in
the ``sps-api-paste.ini`` file, looking like so::

  [pipeline:sps-api-caching]
  pipeline = versionnegotiation context cache apiv1app

To enable the above application pipeline, in your main ``sps-api.conf``
...
directly interact with cache demos. Use this flavor in place of the
...
.. note::
  ...
  These configuration options must be set in both the sps-cache
  and sps-api configuration files.
...
Default: ``/var/lib/sps/image-cache``
...
that will be used to store the cached demos information. The database
...
Size, in bytes, that the image cache should be constrained to. demos files
...
``sps-cache-pruner`` executable is what prunes the image cache to be equal
to or less than this value. The ``sps-cache-pruner`` executable is designed
...
Configuring the sps Registry
-------------------------------

There are a number of configuration options in sps that control how
this registry server operates. These configuration options are specified in the
``sps-registry.conf`` config file in the section ``[DEFAULT]``.

**IMPORTANT NOTE**: The sps-registry service is only used in conjunction
with the sps-api service when clients are using the v1 REST API. See
`Configuring sps APIs`_ for more info.
...
command-line for the ``sps-manage`` program.
...
If the sps API server parameter ``enable_v1_api`` has been set to ``True`` the
``enable_v1_registry`` has to be ``True`` as well.
If the sps API server parameter ``enable_v2_api`` has been set to ``True`` and
the parameter ``data_api`` has been set to ``sps.db.registry.api`` the
...
sps can optionally generate notifications to be logged or sent to
...
For more information :doc:`sps notifications <notifications>`
...
when sps has been configured to use the ``qpid`` notification strategy.
...
This is the username that sps will use to authenticate with the Qpid
...
the sps API server about the policies to use.

* ``policy_file=PATH``

Optional. Default: Looks for a file called ``policy.json`` or
``sps.policy.json`` in standard configuration directories.
...
Configuring sps Property Protections
---------------------------------------
...
location for this file can be specified in the ``sps-api.conf`` config file
in the section ``[DEFAULT]``. **If an incorrect value is specified, sps api
...
Configuring sps APIs
-----------------------

The sps-api service implents versions 1 and 2 of the OpenStack
demos API. Disable either version of the demos API using the
following options:
...
sps-registry service while the v2 API is not. This means that
in order to use the v2 API, you must copy the necessary sql
configuration from your sps-registry service to your
sps-api configuration file.

Configuring sps Tasks
------------------------

sps Tasks are implemented only for version 2 of the OpenStack demos API.

``Please be aware that sps tasks are currently a work in progress
