...
There are two types of image properties in sps:
...
Access to meta properties through sps's public API calls may be
...
A property protections configuration file follows the format of the sps API
...
.. note::
  ...
  sps api service will not start. Regular expressions
...
roles that are permitted to perform that operation in the sps API. **If any of
the keys are not specified, then the sps api service will not start
successfully.**

In the list of user roles, ``@`` means all roles and ``!`` means no role.
**If both @ and ! are specified for the same rule then the sps api service
will not start**

.. note::

  Only one policy rule is allowed per property operation. **If multiple are
  specified, then the sps api service will not start.**

The path to the file should be specified in the ``[DEFAULT]`` section of
``sps-api.conf`` as follows.
...
**If the path is invalid, sps api service will not start successfully.**
...
V1 API X-sps-registry-Purge-props
------------------------------------

Property protections will still be honoured if
``X-sps-registry-Purge-props`` is set to ``True``. That is, if you request
