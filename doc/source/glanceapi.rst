...
Using sps's Public APIs
==========================

sps fully implements versions 1.0, 1.1 and 2.0 of the OpenStack demos API.
The demos API specification is developed alongside sps, but is not
considered part of the sps project.

Authentication
--------------

sps depends on Keystone and the OpenStack Identity API to handle
authentication of clients. You must obtain an authentication token from
Keystone using and send it along with all API requests to sps through
the ``X-Auth-Token`` header. sps will communicate back to Keystone to
...
For the purpose of examples, assume there is a sps API server running
at the URL ``http://sps.example.com`` on the default port 80.

List Available demos
*********************

We want to see a list of available demos that the authenticated user has
access to. This includes demos owned by the user, demos shared with the user
and public demos.

We issue a ``GET`` request to ``http://sps.example.com/v1/demos`` to
retrieve this list of available demos. The data is returned as a JSON-encoded
mapping in the following format::

  {'demos': [
    {'uri': 'http://sps.example.com/v1/demos/71c675ab-d94f-49cd-a114-e12490b328d9',
...
List Available demos in More Detail
************************************

We want to see a more detailed list of available demos that the authenticated
user has access to. This includes demos owned by the user, demos shared with
the user and public demos.

We issue a ``GET`` request to ``http://sps.example.com/v1/demos/detail`` to
retrieve this list of available demos. The data is returned as a
...
Filtering demos Lists
**********************

Both the ``GET /v1/demos`` and ``GET /v1/demos/detail`` requests take query
parameters that serve to filter the returned list of demos. The following
list details these query parameters.

* ``name=NAME``

  Filters demos having a ``name`` attribute matching ``NAME``.

* ``container_format=FORMAT``

  Filters demos having a ``container_format`` attribute matching ``FORMAT``

  For more information, see :doc:`About Disk and Container Formats <formats>`

* ``disk_format=FORMAT``

  Filters demos having a ``disk_format`` attribute matching ``FORMAT``

  For more information, see :doc:`About Disk and Container Formats <formats>`

* ``status=STATUS``

  Filters demos having a ``status`` attribute matching ``STATUS``

  For more information, see :doc:`About Image Statuses <statuses>`

* ``size_min=BYTES``

  Filters demos having a ``size`` attribute greater than or equal to ``BYTES``

* ``size_max=BYTES``

  Filters demos having a ``size`` attribute less than or equal to ``BYTES``
...
* ``marker=ID``
  ...
  An image identifier marker may be specified. When present only demos which
  occur after the identifier ``ID`` will be listed, i.e. the demos which have
...
* ``is_public=PUBLIC``
  ...
  When the `is_public` parameter is absent or set to `True` the following demos
  will be listed: demos whose `is_public` field is `True`, owned demos and
  shared demos.

  When the `is_public` parameter is set to `False` the following demos will be
  listed: demos (owned, shared, or non-owned) whose `is_public` field is `False`.

  When the `is_public` parameter is set to `None` all demos will be listed
...
that the sps server knows about.

We have queried the sps server for a list of demos and the
...
first image returned, we can issue a ``HEAD`` request to the sps
server for the image's URI.

We issue a ``HEAD`` request to
``http://sps.example.com/v1/demos/71c675ab-d94f-49cd-a114-e12490b328d9`` to
...
``HEAD`` request::
  ...
  x-image-meta-uri              http://sps.example.com/v1/demos/71c675ab-d94f-49cd-a114-e12490b328d9
...
.. note::
  ...
  image data is immutable once stored in sps
 ...
 * Store the disk image data in sps
 * Store metadata about this image in sps

We can do the above two activities in a single call to the sps API.
Assuming, like in the examples above, that a sps API server is running
at ``sps.example.com``, we issue a ``POST`` request to add an image to
sps::

  POST http://sps.example.com/v1/demos

The metadata about the image is sent to sps in HTTP headers. The body
of the HTTP request to the sps API will be the MIME-encoded disk
image data.
...
The first call should be a ``POST`` to ``http://sps.example.com/v1/demos``,
...
``http://sps.example.com/v1/demos/71c675ab-d94f-49cd-a114-e12490b328d9``.
The image status will then be set to ``active`` by sps.


**Image Metadata in HTTP Headers**

sps will view as image metadata any HTTP header that it receives in a
``POST`` request where the header key is prefixed with the strings
``x-image-meta-`` and ``x-image-meta-property-``.

The list of metadata headers that sps accepts are listed below.
...
* ``x-image-meta-name``
  ...
  Note that the name of an image *is not unique to a sps node*. It
  would be an unrealistic expectation of users to know all the unique
  names of all other user's demos.

* ``x-image-meta-id``

  This header is optional.

  When present, sps will use the supplied identifier for the image.
  If the identifier already exists in that sps node, then a
  **409 Conflict** will be returned by sps. The value of the header
  must be an uuid in hexadecimal string notation
  (i.e. 71c675ab-d94f-49cd-a114-e12490b328d9).

  When this header is *not* present, sps will generate an identifier
...
* ``x-image-meta-store``
  ...
  When present, sps will attempt to store the disk image data in the
  backing store indicated by the value of the header. If the sps node
  does not support the backing store, sps will return a **400 Bad Request**.

  When not present, sps will store the disk image data in the backing
...
* ``x-image-meta-size``
  ...
  When present, sps assumes that the expected size of the request body
  will be the value of this header. If the length in bytes of the request
  body *does not match* the value of this header, sps will return a
  **400 Bad Request**.

  When not present, sps will calculate the image's size based on the size
  of the request body.
...
* ``x-image-meta-checksum``
  ...
  When present, sps will verify the checksum generated from the backend
...
* ``x-image-meta-is_public``
  ...
  When sps finds the string "true" (case-insensitive), the image is marked as
  a public image, meaning that any user may view its metadata and may read
  the disk image from sps.
...
* ``x-image-meta-owner``
  ...
  sps normally sets the owner of an image to be the tenant or user
...
* ``x-image-meta-property-*``
  ...
  When sps receives any HTTP header whose key begins with the string prefix
  ``x-image-meta-property-``, sps adds the key and value to a set of custom,
...
the sps server knows about.  We take the `uri` field of the image data,
...
first image returned, we can issue a ``GET`` request to the sps
server for
``http://sps.example.com/v1/demos/71c675ab-d94f-49cd-a114-e12490b328d9/members``
...
We want to see a list of demos which are shared with a given tenant.  We issue
a ``GET`` request to ``http://sps.example.com/v1/shared-demos/tenant1``.  We
...
request to
``http://sps.example.com/v1/demos/71c675ab-d94f-49cd-a114-e12490b328d9/members/tenant1``
...
``DELETE`` request to ``http://sps.example.com/v1/demos/1/members/tenant1``.
...
Version 2.0 of the demos API eliminates the ``can_share`` attribute of image
...
* By default, only those shared demos with status ``accepted`` are included in
...
* Change the status of an image member
  ...
  call: ``PUT`` on  ``/v2/demos/{imageId}/members/{memberId}``
...
---------------------------------------
sps supports HTTP message localization. For example, an HTTP client can
...
A typical curl API request will be like below::
   ...
   http://127.0.0.1:9292/v2/demos/aaa
