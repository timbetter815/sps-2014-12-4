...
function run_tests {
  ...
  if [ setup.cfg -nt sps.egg-info/entry_points.txt ]
  then
  ...
  if [ $coverage -eq 1 ]; then
    ...
    ${wrapper} coverage html --include='sps/*' --omit='sps/openstack/common/*' -d covhtml -i
  fi
...
function run_pep8 {
  ...
  bash -c "${wrapper} find sps -type f -regex '.*\.pot?' -print0|${wrapper} xargs --null -n 1 ${wrapper} msgfmt --check-format -o /dev/null"
}


TESTRTESTS="python -m sps.openstack.common.lockutils python setup.py testr"
