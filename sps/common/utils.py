...
from sps.openstack.common import network_utils
...
sps_TEST_SOCKET_FD_STR = 'sps_TEST_SOCKET_FD'
    ...
    def __iter__(self):
        for chunk in self.data:
            ...
            if self.bytes_read > self.limit:
                raise exception.demosizeLimitExceeded()
            else:
                yield chunk
...
def add_features_to_http_headers(features, headers):
    """
    Adds additional headers representing sps features to be enabled.
...
class PrettyTable(object):
    """Creates an ASCII art table for use in bin/sps
    ...
    def __get_backend(self):
        if not self.__backend:
            ...
            if backend_name not in self.__backends:
                ...
                raise exception.spsException(msg)
...
def get_test_suite_socket():
    global sps_TEST_SOCKET_FD_STR
    if sps_TEST_SOCKET_FD_STR in os.environ:
        fd = int(os.environ[sps_TEST_SOCKET_FD_STR])
        sock = socket.fromfd(fd, socket.AF_INET, socket.SOCK_STREAM)
        sock = socket.SocketType(_sock=sock)
        sock.listen(CONF.backlog)
        del os.environ[sps_TEST_SOCKET_FD_STR]
        os.close(fd)
        return sock
    return None
