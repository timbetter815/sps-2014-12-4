...
class BaseClient(object):
    ...
    def get_connect_kwargs(self):
        ...
        if self.use_ssl:
            if self.key_file is None:
                self.key_file = os.environ.get('sps_CLIENT_KEY_FILE')
            if self.cert_file is None:
                self.cert_file = os.environ.get('sps_CLIENT_CERT_FILE')
            if self.ca_file is None:
                self.ca_file = os.environ.get('sps_CLIENT_CA_FILE')
            ...
            if self.cert_file is not None and self.key_file is None:
                msg = _("You have selected to use SSL in connecting, "
                        ...
                        "sps_CLIENT_KEY_FILE environ variable")
            ...
            if self.key_file is not None and self.cert_file is None:
                msg = _("You have selected to use SSL in connecting, "
                        ...
                        "sps_CLIENT_CERT_FILE environ variable")
                raise exception.ClientConnectionError(msg)
    ...
    def set_auth_token(self, auth_tok):
        ...
        """
        # FIXME(sirp): Nova image/sps.py currently calls this. Since this
        # method isn't really doing anything useful[1], we should go ahead and
        # rip it out, first in Nova, then here. Steps:
        #
        #       1. Change auth_tok in sps to auth_token
        #       2. Change image/sps.py in Nova to use client.auth_token
    ...
    def _do_request(self, method, url, body, headers):
        ...
        any returned HTTP error status codes to OpenStack/sps exceptions
