...
# FIXME(sirp): When we have sps-admin we can consider merging this into it
# Perhaps for consistency with Nova, we would then rename sps-admin ->
# sps-manage (or the other way around)
...
def main():
    ...
    try:
        cfg_files = cfg.find_config_files(project='sps',
                                          prog='sps-registry')
        cfg_files.extend(cfg.find_config_files(project='sps',
                                               prog='sps-api'))
    ...
    except exception.spsException as e:
        sys.exit("ERROR: %s" % e)
