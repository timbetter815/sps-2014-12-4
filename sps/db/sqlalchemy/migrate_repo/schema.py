...
def from_migration_import(module_name, fromlist):
    ...
    :param module_name: name of migration module to import from
        (ex: 001_add_demos_table)
    :param fromlist: list of items to import (ex: define_demos_table)
    ...
    Example Usage:
        ...
        (define_demos_table,) = from_migration_import(
            '001_add_demos_table', ['define_demos_table'])

        demos = define_demos_table(meta)

        # Refer to demos table
    """
    module_path = 'sps.db.sqlalchemy.migrate_repo.versions.%s' % module_name
