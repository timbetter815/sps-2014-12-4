...
from sps.api.middleware import context
from sps.api.v1 import router
import sps.common.client
from sps.registry.api import v1 as rserver
...
def stub_out_registry_and_store_server(stubs, base_dir, **kwargs):
    ...
    that a real sps server does not need to be up and
    running
    """
    ...
    class FakespsConnection(object):
    ...
    def fake_sendable(self, body):
        ...
        if force is None:
            ...
            if force:
                assert sps.common.client.SENDFILE_SUPPORTED
            return force

    stubs.Set(sps.common.client.BaseClient, 'get_connection_type',
              fake_get_connection_type)
    setattr(sps.common.client.BaseClient, '_stub_orig_sendable',
            sps.common.client.BaseClient._sendable)
    stubs.Set(sps.common.client.BaseClient, '_sendable',
              fake_sendable)
...
def stub_out_registry_server(stubs, **kwargs):
    ...
    that a real sps Registry server does not need to be up and
    running
    """
    def fake_get_connection_type(client):
        """
