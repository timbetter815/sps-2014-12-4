...
from sps.api import authorization
from sps.common import auth
from sps.common import exception
import sps.domain
from sps.openstack.common import jsonutils
from sps.openstack.common import timeutils
from sps.tests.unit import utils as unittest_utils
from sps.tests import utils
...
class V2Token(object):
    ...
    def add_service(self, s_type, region_list=None):
        ...
        catalog = self.tok['access']['serviceCatalog']
        service_type = {"type": s_type, "name": "sps"}
        catalog.append(service_type)
        service = catalog[-1]
        endpoint_list = []
...
class TestImageMutability(utils.BaseTestCase):
    ...
    def _is_mutable(self, tenant, owner, is_admin=False):
        context = sps.context.RequestContext(tenant=tenant,
                                                is_admin=is_admin)
...
class TestImmutableImage(utils.BaseTestCase):
    def setUp(self):
        ...
        image_factory = sps.domain.ImageFactory()
        self.context = sps.context.RequestContext(tenant=TENANT1)
    ...
    def test_get_data(self):
        ...
        image = sps.api.authorization.ImmutableImageProxy(
...
class TestImageFactoryProxy(utils.BaseTestCase):
    def setUp(self):
        ...
        factory = sps.domain.ImageFactory()
        self.context = sps.context.RequestContext(tenant=TENANT1)
...
class TestImageRepoProxy(utils.BaseTestCase):
    ...
    def test_list(self):
        ...
        self.assertEqual(demos[0].image_id, self.fixtures[0].image_id)
        self.assertRaises(exception.Forbidden,
                          setattr, demos[1], 'name', 'Wally')
        self.assertRaises(exception.Forbidden,
                          setattr, demos[2], 'name', 'Calvin')


class TestImmutableTask(utils.BaseTestCase):
    def setUp(self):
        super(TestImmutableTask, self).setUp()
        task_factory = sps.domain.TaskFactory()
        self.context = sps.context.RequestContext(tenant=TENANT2)
        task_type = 'import'
        owner = TENANT2
...
class TestTaskFactoryProxy(utils.BaseTestCase):
    def setUp(self):
        ...
        factory = sps.domain.TaskFactory()
        self.context = sps.context.RequestContext(tenant=TENANT1)
        self.context_owner_is_none = sps.context.RequestContext()
