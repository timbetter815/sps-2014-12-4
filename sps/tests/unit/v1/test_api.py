...
import sps.api
import sps.api.common
from sps.api.v1 import router
from sps.api.v1 import upload_utils
import sps.common.config
from sps.common import exception
import sps.context
from sps.db.sqlalchemy import api as db_api
from sps.db.sqlalchemy import models as db_models
from sps.openstack.common import jsonutils
from sps.openstack.common import timeutils

import sps.registry.client.v1.api as registry
import sps.store.filesystem
from sps.store import http
from sps.tests.unit import base
import sps.tests.unit.utils as unit_test_utils
...
class TestspsAPI(base.IsolatedUnitTest):
    def setUp(self):
        """Establish a clean test environment"""
        super(TestspsAPI, self).setUp()
        ...
        self.FIXTURES = [
            ...
            {'id': UUID2,
             ...
             'locations': [{'url': "file:///%s/%s" % (self.test_dir, UUID2),
                            'metadata': {}}],
             'properties': {}}]
        self.context = sps.context.RequestContext(is_admin=True)
        db_api.get_engine()
    ...
    def tearDown(self):
        ...
        super(TestspsAPI, self).tearDown()
        self.destroy_fixtures()
    ...
    def test_bad_min_disk_size_update(self):
        ...
        req = webob.Request.blank("/demos/%s" % image_id)
        req.method = 'PUT'
    ...
    def test_add_image_no_location_no_image_as_body(self):
        ...
        req = webob.Request.blank("/demos/%s" % image_id)
        req.method = 'PUT'
        req.headers['x-image-meta-location'] = 'http://localhost:0/demos/123'
    ...
    def test_add_image_basic_file_store(self):
        ...
        res_body = jsonutils.loads(res.body)['image']
        self.assertIn('/demos/%s' % res_body['id'], res.headers['location'])
        self.assertEqual('active', res_body['status'])
        image_id = res_body['id']
        ...
        req = webob.Request.blank("/demos/%s" % image_id)
        req.method = 'PUT'
        req.headers['x-image-meta-location'] = 'http://example.com/demos/123'
        res = req.get_response(self.api)
        self.assertEqual(res.status_int, 400)
    ...
    def test_add_copy_from_image_unauthorized(self):
        ...
        fixture_headers = {'x-image-meta-store': 'file',
                           'x-image-meta-disk-format': 'vhd',
                           'x-sps-api-copy-from': 'http://sps.com/i.ovf',
                           'x-image-meta-container-format': 'ovf',
    ...
    def test_add_copy_from_with_nonempty_body(self):
        ...
        fixture_headers = {'x-image-meta-store': 'file',
                           'x-image-meta-disk-format': 'vhd',
                           'x-sps-api-copy-from': 'http://a/b/c.ovf',
                           'x-image-meta-container-format': 'ovf',
                           'x-image-meta-name': 'fake image #F'}
    ...
    def test_update_copy_from_upload_image_unauthorized(self):
        ...
        req.method = 'PUT'
        req.headers['Content-Type'] = 'application/octet-stream'
        req.headers['x-sps-api-copy-from'] = 'http://sps.com/i.ovf'
        res = req.get_response(self.api)
        self.assertEqual(res.status_int, 403)
    ...
    def test_update_deleted_image(self):
        ...
        req = webob.Request.blank("/demos/%s" % UUID2)
        req.method = 'DELETE'
        res = req.get_response(self.api)
        self.assertEqual(res.status_int, 200)
    ...
    @mock.patch.object(sps.store.filesystem.Store, 'delete')
    ...
    def test_upload_to_image_status_saving(self):
        ...
        image_id = res_body['id']
        self.assertIn('/demos/%s' % image_id, res.headers['location'])
    ...
    def test_delete_during_image_upload(self):
        ...
        self.stubs.Set(sps.api.v1.upload_utils, 'initiate_deletion',
                       mock_initiate_deletion)

        orig_update_image_metadata = registry.update_image_metadata
        ctlr = sps.api.v1.controller.BaseController
    ...
    def test_disable_purge_props(self):
        """
        Test the special x-sps-registry-purge-props header controls
        ...
        # passing the x-sps-registry-purge-props header and
    ...
    def test_publicize_image_unauthorized(self):
        ...
        res_body = jsonutils.loads(res.body)['image']
        req = webob.Request.blank("/demos/%s" % res_body['id'])
        req.method = 'PUT'
    ...
    def test_update_non_existing_image(self):
        ...
        req = webob.Request.blank("demos/%s" % _gen_uuid)
        req.method = 'PUT'
        req.body = 'test'
    ...
    def test_get_index_sort_name_asc(self):
        """
        Tests that the /demos registry API returns list of
        public demos sorted alphabetically by name in
        ascending order.
        """
        UUID3 = _gen_uuid()
        ...
        req = webob.Request.blank('/demos?sort_key=name&sort_dir=asc')
        res = req.get_response(self.api)
        self.assertEqual(res.status_int, 200)
        res_dict = jsonutils.loads(res.body)

        demos = res_dict['demos']
        self.assertEqual(len(demos), 3)
        self.assertEqual(demos[0]['id'], UUID3)
        self.assertEqual(demos[1]['id'], UUID2)
        self.assertEqual(demos[2]['id'], UUID4)

    def test_get_details_filter_changes_since(self):
        """
        Tests that the /demos/detail registry API returns list of
        public demos that have a size less than or equal to size_max
        """
        ...
        # Check a standard list, 4 demos in db (2 deleted)
        req = webob.Request.blank('/demos/detail')
        res = req.get_response(self.api)
        self.assertEqual(res.status_int, 200)
        res_dict = jsonutils.loads(res.body)
        demos = res_dict['demos']
        self.assertEqual(len(demos), 2)
        self.assertEqual(demos[0]['id'], UUID4)
        self.assertEqual(demos[1]['id'], UUID2)

        # Expect 3 demos (1 deleted)
        req = webob.Request.blank('/demos/detail?changes-since=%s' % iso1)
        res = req.get_response(self.api)
        self.assertEqual(res.status_int, 200)
        res_dict = jsonutils.loads(res.body)
        demos = res_dict['demos']
        self.assertEqual(len(demos), 3)
        self.assertEqual(demos[0]['id'], UUID4)
        self.assertEqual(demos[1]['id'], UUID3)  # deleted
        self.assertEqual(demos[2]['id'], UUID2)

        # Expect 1 demos (0 deleted)
        req = webob.Request.blank('/demos/detail?changes-since=%s' % iso2)
        res = req.get_response(self.api)
        self.assertEqual(res.status_int, 200)
        res_dict = jsonutils.loads(res.body)
        demos = res_dict['demos']
        ...
        # Expect 1 demos (0 deleted)
        req = webob.Request.blank('/demos/detail?changes-since=%s' %
                                  hour_before)
        res = req.get_response(self.api)
        self.assertEqual(res.status_int, 200)
        res_dict = jsonutils.loads(res.body)
        ...
        # Expect 0 demos (0 deleted)
        req = webob.Request.blank('/demos/detail?changes-since=%s' % iso4)
        res = req.get_response(self.api)
        self.assertEqual(res.status_int, 200)
        res_dict = jsonutils.loads(res.body)
        demos = res_dict['demos']
        ...
        req = webob.Request.blank('/demos/detail?changes-since=')
        res = req.get_response(self.api)
        self.assertEqual(res.status_int, 400)

    def test_get_demos_bad_urls(self):
        """Check that routes collections are not on (LP bug 1185828)"""
        req = webob.Request.blank('/demos/detail.xxx')
        res = req.get_response(self.api)
        self.assertEqual(res.status_int, 404)

        req = webob.Request.blank('/demos.xxx')
        res = req.get_response(self.api)
        self.assertEqual(res.status_int, 404)

        req = webob.Request.blank('/demos/new')
        res = req.get_response(self.api)
        self.assertEqual(res.status_int, 404)

        req = webob.Request.blank("/demos/%s/members" % UUID1)
        res = req.get_response(self.api)
        self.assertEqual(res.status_int, 200)

        req = webob.Request.blank("/demos/%s/members.xxx" % UUID1)
        res = req.get_response(self.api)
        self.assertEqual(res.status_int, 404)
    ...
    def test_get_index_filter_on_user_defined_properties(self):
        ...
        req = webob.Request.blank('/demos?property-distro=ubuntu')
        res = req.get_response(self.api)
        self.assertEqual(res.status_int, 200)
        demos = jsonutils.loads(res.body)['demos']
        self.assertEqual(len(demos), 2)
        self.assertEqual(demos[0]['id'], image2_id)
        self.assertEqual(demos[1]['id'], image1_id)

        # Test index with filter containing one user-defined property but
        # non-existent value. Filter is 'property-distro=fedora'.
        # Verify neither demos are returned
        req = webob.Request.blank('/demos?property-distro=fedora')
        res = req.get_response(self.api)
        self.assertEqual(res.status_int, 200)
        demos = jsonutils.loads(res.body)['demos']
        self.assertEqual(len(demos), 0)
        ...
        req = webob.Request.blank('/demos?property-arch=i386')
        res = req.get_response(self.api)
        self.assertEqual(res.status_int, 200)
        demos = jsonutils.loads(res.body)['demos']
        self.assertEqual(len(demos), 1)
        self.assertEqual(demos[0]['id'], image1_id)

        # Test index with filter containing one user-defined property but
        # unique value. Filter is 'property-arch=x86_64'.
        # Verify only image1 is returned.
        req = webob.Request.blank('/demos?property-arch=x86_64')
        res = req.get_response(self.api)
        self.assertEqual(res.status_int, 200)
        demos = jsonutils.loads(res.body)['demos']
        self.assertEqual(len(demos), 1)
        ...
        req = webob.Request.blank('/demos?property-foo=bar')
        res = req.get_response(self.api)
        self.assertEqual(res.status_int, 200)
        demos = jsonutils.loads(res.body)['demos']
        self.assertEqual(len(demos), 1)
        ...
        # Verify neither demos are returned.
        req = webob.Request.blank('/demos?property-foo=baz')
        res = req.get_response(self.api)
        self.assertEqual(res.status_int, 200)
        demos = jsonutils.loads(res.body)['demos']
        self.assertEqual(len(demos), 0)
        ...
        # Verify neither demos are returned.
        req = webob.Request.blank('/demos?property-arch=random&'
                                  'property-distro=ubuntu')
        res = req.get_response(self.api)
        self.assertEqual(res.status_int, 200)
        demos = jsonutils.loads(res.body)['demos']
        ...
        # Verify neither demos are returned.
        req = webob.Request.blank('/demos?property-boo=far&'
                                  'property-poo=far')
        res = req.get_response(self.api)
        self.assertEqual(res.status_int, 200)
        demos = jsonutils.loads(res.body)['demos']
    ...
    def test_get_demos_detailed_unauthorized(self):
        rules = {"get_demos": '!'}
        self.set_policy_rules(rules)
        req = webob.Request.blank('/demos/detail')
        res = req.get_response(self.api)
    ...
    def test_get_demos_unauthorized(self):
        rules = {"get_demos": '!'}
        self.set_policy_rules(rules)
        req = webob.Request.blank('/demos')
        res = req.get_response(self.api)
    ...
    def test_store_location_not_revealed(self):
        ...
        for url in ('/demos', '/demos/detail'):
            ...
            demos = res_dict['demos']
            num_locations = sum([1 for record in demos
                                if 'location' in record.keys()])
            self.assertEqual(0, num_locations, demos)
    ...
    def test_etag_equals_checksum_header(self):
        ...
        expected_headers = {'x-image-meta-checksum': image_checksum,
                            'etag': image_checksum}
        req = webob.Request.blank("/demos/%s" % image['id'])
        req.method = 'HEAD'
        res = req.get_response(self.api)
        self.assertEqual(res.status_int, 200)
    ...
    def test_get_details_invalid_marker(self):
        """
        Tests that the /demos/detail registry API returns a 400
        when an invalid marker is provided
        """
        req = webob.Request.blank('/demos/detail?marker=%s' % _gen_uuid())
        res = req.get_response(self.api)
        self.assertEqual(res.status_int, 400)

    def test_get_image_members(self):
        """
        Tests members listing for existing demos
        """
        req = webob.Request.blank('/demos/%s/members' % UUID2)
        req.method = 'GET'
    ...
    def test_get_image_members_not_existing(self):
        ...
        non-existing image
        """
        req = webob.Request.blank('/demos/%s/members' % _gen_uuid())
        req.method = 'GET'
    ...
    def test_add_member_positive(self):
        ...
        req = webob.Request.blank('/demos/%s/members/pattieblack' % UUID2)
        req.method = 'PUT'

        res = req.get_response(self.api)
        self.assertEqual(res.status_int, 204)

    def test_get_member_demos(self):
        """
        Tests image listing for members
        """
        req = webob.Request.blank('/shared-demos/pattieblack')
        req.method = 'GET'
        ...
        memb_list = jsonutils.loads(res.body)
        num_members = len(memb_list['shared_demos'])
        self.assertEqual(num_members, 0)
    ...
    def test_add_member_non_existing_image(self):
        ...
        self.api = test_utils.FakeAuthMiddleware(
            test_router, is_admin=True)
        test_uri = '/demos/%s/members/pattieblack'
    ...
    def test_add_member_forbidden_by_policy(self):
        ...
        self.api = test_utils.FakeAuthMiddleware(router.API(self.mapper),
                                                 is_admin=True)
        req = webob.Request.blank('/demos/%s/members/pattieblack' % UUID1)
        req.method = 'PUT'
    ...
    def test_delete_non_exist_member(self):
        ...
        api = test_utils.FakeAuthMiddleware(
            test_router, is_admin=True)
        req = webob.Request.blank('/demos/%s/members/test_user' % UUID2)
        req.method = 'DELETE'
        res = req.get_response(api)
        self.assertEqual(res.status_int, 404)
    ...
    def test_delete_image_member(self):
        ...
        # Add member to image:
        fixture = dict(can_share=True)
        test_uri = '/demos/%s/members/test_add_member_positive'
...
class Testdemoserializer(base.IsolatedUnitTest):
    def setUp(self):
        """Establish a clean test environment"""
        super(Testdemoserializer, self).setUp()
        self.receiving_user = 'fake_user'
        self.receiving_tenant = 2
        self.context = sps.context.RequestContext(
            is_admin=True,
            user=self.receiving_user,
            tenant=self.receiving_tenant)
        self.serializer = sps.api.v1.demos.demoserializer()
    ...
    def test_meta(self):
        exp_headers = {'x-image-meta-id': UUID2,
                       'x-image-meta-location': 'file:///tmp/sps-tests/2',
    ...
    def test_meta_utf8(self):
        ...
        FIXTURE = {
            'image_meta': {
                ...
                'location': u"file:///tmp/sps-tests/2",
                'properties': {
    ...
    def test_image_send_notification(self):
        ...
        sps.api.common.image_send_notification(19, 19, image_meta, req,
    ...
    def test_image_send_notification_error(self):
        ...
        sps.api.common.image_send_notification(17, 19, image_meta, req,
                                                  self.serializer.notifier)
    ...
    def test_redact_location(self):
        ...
        tmp_image_meta = sps.api.v1.demos.redact_loc(image_meta)
...
class TestFilterValidator(base.IsolatedUnitTest):
    def test_filter_validator(self):
        self.assertFalse(sps.api.v1.filters.validate('size_max', -1))
        self.assertTrue(sps.api.v1.filters.validate('size_max', 1))
        self.assertTrue(sps.api.v1.filters.validate('protected', 'True'))
        self.assertTrue(sps.api.v1.filters.validate('protected', 'FALSE'))
        self.assertFalse(sps.api.v1.filters.validate('protected', '-1'))
...
class TestAPIProtectedProps(base.IsolatedUnitTest):
    ...
    def _create_admin_image(self, props=None):
        if props is None:
            props = {}
        request = unit_test_utils.get_fake_request(path='/demos')
    ...
    def test_prop_protection_with_create_and_permitted_role(self):
        ...
        another_request = unit_test_utils.get_fake_request(
            path='/demos/%s' % image_id, method='PUT')
    ...
    def test_prop_protection_with_show_and_permitted_role(self):
        ...
        image_id = self._create_admin_image(
            {'x-image-meta-property-x_owner_foo': 'bar'})
        another_request = unit_test_utils.get_fake_request(
            method='HEAD', path='/demos/%s' % image_id)
        headers = {'x-auth-token': 'user:tenant:member'}
        for k, v in six.iteritems(headers):
            another_request.headers[k] = v
    ...
    def test_prop_protection_with_get_and_permitted_role(self):
        ...
        image_id = self._create_admin_image(
            {'x-image-meta-property-x_owner_foo': 'bar'})
        another_request = unit_test_utils.get_fake_request(
            method='GET', path='/demos/%s' % image_id)
        headers = {'x-auth-token': 'user:tenant:member'}
        for k, v in six.iteritems(headers):
            another_request.headers[k] = v
        res2 = another_request.get_response(self.api)
    ...
    def test_prop_protection_with_detail_and_permitted_role(self):
        ...
        another_request = unit_test_utils.get_fake_request(
            method='GET', path='/demos/detail')
        headers = {'x-auth-token': 'user:tenant:member'}
        for k, v in six.iteritems(headers):
            another_request.headers[k] = v
        output = another_request.get_response(self.api)
        self.assertEqual(output.status_int, 200)
        res_body = jsonutils.loads(output.body)['demos'][0]
        self.assertEqual(res_body['properties']['x_owner_foo'], 'bar')
    ...
    def test_prop_protection_with_delete_and_permitted_role(self):
        ...
        another_request = unit_test_utils.get_fake_request(
            path='/demos/%s' % image_id, method='PUT')
        headers = {'x-auth-token': 'user:tenant:member',
                   'X-sps-Registry-Purge-Props': 'True'}
        for k, v in six.iteritems(headers):
            another_request.headers[k] = v
        output = another_request.get_response(self.api)
        res_body = jsonutils.loads(output.body)['image']
    ...
    def test_read_protected_props_leak_with_update(self):
        ...
        headers = {'x-auth-token': 'user:tenant:spl_role',
                   ...
                   'X-sps-Registry-Purge-Props': 'False'}
        for k, v in six.iteritems(headers):
            another_request.headers[k] = v
        output = another_request.get_response(self.api)
        res_body = jsonutils.loads(output.body)['image']
