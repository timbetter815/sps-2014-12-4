...
class TestSizeCheckedIter(testtools.TestCase):
    ...
    def test_uniform_chunk_size(self):
        ...
        checked_image = sps.api.common.size_checked_iter(
    ...
    def test_too_many_chunks(self):
        ...
        self.assertRaises(exception.spsException, checked_image.next)

    def test_too_few_chunks(self):
        resp = self._get_webob_response()
        meta = self._get_image_metadata()
        checked_image = sps.api.common.size_checked_iter(resp, meta, 6,
                                                            ['AB', 'CD'],
                                                            None)
    ...
    def test_too_much_data(self):
        resp = self._get_webob_response()
        meta = self._get_image_metadata()
        checked_image = sps.api.common.size_checked_iter(resp, meta, 3,
                                                            ['AB', 'CD'],
                                                            None)
...
class TestMalformedRequest(test_utils.BaseTestCase):
    def setUp(self):
        ...
        self.config(flavor='',
                    group='paste_deploy',
                    config_file='etc/sps-api-paste.ini')
        self.api = config.load_paste_app('sps-api')

    def test_redirect_incomplete_url(self):
        """Test sps redirects /v# to /v#/ with correct Location header"""
