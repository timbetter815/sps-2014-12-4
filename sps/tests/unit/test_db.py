...
@mock.patch('sps.openstack.common.importutils.import_module')
class TestDbUtilities(test_utils.BaseTestCase):
    ...
    def test_get_api_calls_configure_if_present(self, import_module):
        ...
        self.assertEqual(sps.db.get_api(), self.api)
...
class TestImageRepo(test_utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.image_repo = sps.db.ImageRepo(self.context, self.db)
        self.image_factory = sps.domain.ImageFactory()
        self._create_demos()
        self._create_image_members()

    def _create_demos(self):
        self.db.reset()
        self.demos = [
        ...
        [self.db.image_create(None, image) for image in self.demos]
    ...
    def test_list(self):
        demos = self.image_repo.list()
        image_ids = set([i.image_id for i in demos])
    ...
    def _do_test_list_status(self, status, expected):
        ...
        self.image_repo = sps.db.ImageRepo(self.context, self.db)
        demos = self.image_repo.list(member_status=status)
        self.assertEqual(expected, len(demos))
    ...
    def test_list_with_marker(self):
        full_demos = self.image_repo.list()
        full_ids = [i.image_id for i in full_demos]
        marked_demos = self.image_repo.list(marker=full_ids[0])
        actual_ids = [i.image_id for i in marked_demos]
        self.assertEqual(actual_ids, full_ids[1:])

    def test_list_with_last_marker(self):
        demos = self.image_repo.list()
        marked_demos = self.image_repo.list(marker=demos[-1].image_id)
        self.assertEqual(len(marked_demos), 0)

    def test_limited_list(self):
        limited_demos = self.image_repo.list(limit=2)
        self.assertEqual(len(limited_demos), 2)

    def test_list_with_marker_and_limit(self):
        full_demos = self.image_repo.list()
        full_ids = [i.image_id for i in full_demos]
        marked_demos = self.image_repo.list(marker=full_ids[0], limit=1)
        actual_ids = [i.image_id for i in marked_demos]
        self.assertEqual(actual_ids, full_ids[1:2])

    def test_list_private_demos(self):
        filters = {'visibility': 'private'}
        demos = self.image_repo.list(filters=filters)
        image_ids = set([i.image_id for i in demos])
    ...
    def test_list_with_checksum_filter_single_image(self):
        ...
        demos = self.image_repo.list(filters=filters)
        image_ids = list([i.image_id for i in demos])
        self.assertEqual(1, len(image_ids))
        self.assertEqual([UUID1], image_ids)

    def test_list_with_checksum_filter_multiple_demos(self):
    ...
    def test_list_public_demos(self):
    ...
    def test_sorted_list(self):
        demos = self.image_repo.list(sort_key='size', sort_dir='asc')
        image_ids = [i.image_id for i in demos]
...
class TestImageMemberRepo(test_utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.context = sps.context.RequestContext(
            user=USER1, tenant=TENANT1)
        self.image_repo = sps.db.ImageRepo(self.context, self.db)
        self.image_member_factory = sps.domain.ImageMemberFactory()
        self._create_demos()
        self._create_image_members()
        image = self.image_repo.get(UUID1)
        self.image_member_repo = sps.db.ImageMemberRepo(self.context,
                                                           self.db, image)
    ...
    def test_list_no_members(self):
        image = self.image_repo.get(UUID2)
        self.image_member_repo_uuid2 = sps.db.ImageMemberRepo(
...
class TestTaskRepo(test_utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.db = unit_test_utils.FakeDB()
        self.db.reset()
        self.context = sps.context.RequestContext(user=USER1,
                                                     tenant=TENANT1)
        self.task_repo = sps.db.TaskRepo(self.context, self.db)
        self.task_factory = sps.domain.TaskFactory()
