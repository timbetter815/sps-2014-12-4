...
from sps import store
from sps.store import location
from sps.store import sheepdog
from sps.store import vmware_datastore
from sps.tests import stubs
from sps.tests import utils as test_utils

CONF = cfg.CONF
CONF.import_opt('filesystem_store_datadir', 'sps.store.filesystem')
CONF.import_opt('connection', 'sps.openstack.common.db.sqlalchemy.session',
                group='database')
...
class IsolatedUnitTest(StoreClearingUnitTest):
    ...
    def _copy_data_file(self, file_name, dst_dir):
        src_file_name = os.path.join('sps/tests/etc', file_name)
