...
from sps.openstack.common import units
...
def size_checked_iter(response, image_meta, expected_size, image_iter,
    ...
    if expected_size != bytes_written:
        ...
        LOG.error(msg)
        raise exception.spsException(_("Corrupt image download for "
                                          "image %(image_id)s") %
                                        {'image_id': image_id})
...
def get_remaining_quota(context, db_api, image_id=None):
    ...
    of the given size in sps based on their quota and current usage.
    :param context:
